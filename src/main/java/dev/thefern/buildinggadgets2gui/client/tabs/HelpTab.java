...
public class HelpTab extends TabPanel {
    ...
    private void buildHelpContent() {
        ...
        addBullet("Tabs: Schematics, Search, History, Trash, Settings, Help");
        addEmpty();
        ...
        addBullet("Save Schematic - Save clipboard to a file (with optional tags)");
        addEmpty();
        addSubheader("Clipboard Row:");
        addBullet("Shows block count, dimensions, and timestamp");
        addBullet("M (Material) - View required materials list");
        addBullet("T (Tool) - Send clipboard back to gadget");
        ...
        addBullet("# (Hash) - Manage global tags");
        addEmpty();
        addSubheader("File List:");
        addBullet("Click file to select and view info panel");
        addBullet("Click folder to enter it");
        addBullet("X - Delete file/folder (moves to Trash)");
        addBullet("# - Edit tags for the schematic");
        addBullet("M - View materials list");
        addBullet("T - Send schematic to gadget");
        addEmpty();
        
        addHeader("Search Tab (magnifying glass icon)");
        addText("Find schematics across all folders.");
        addEmpty();
        addSubheader("Search Options:");
        addBullet("Type in the search box to filter by name/description");
        addBullet("Click tags to filter by tag (can select multiple)");
        addBullet("Selected tags show a checkmark");
        addBullet("Clear button resets search and tag filters");
        addEmpty();
        addSubheader("Tag Section:");
        addBullet("Scroll with mouse wheel if you have many tags");
        addBullet("Click a tag to toggle it on/off");
        addEmpty();
        addSubheader("Results:");
        addBullet("Shows matching schematics from all folders");
        addBullet("Each result has action buttons: Tags, Material, Tool");
        addBullet("Scroll results with mouse wheel");
        addBullet("Search state is preserved when closing dialogs");
        addEmpty();
        
        addHeader("Tags System");
        addText("Organize schematics with custom tags.");
        addEmpty();
        addSubheader("Managing Tags:");
        addBullet("Click # button in Schematics tab to open Tag Manager");
        addBullet("Create new tags with the input box");
        addBullet("Delete unused tags with the X button");
        addBullet("Rename tags by clicking on them");
        addEmpty();
        addSubheader("Tagging Schematics:");
        addBullet("Add tags when saving a new schematic");
        addBullet("Click # on any schematic to edit its tags");
        addBullet("Tags are shown in the file list and info panel");
        addEmpty();
        
        addHeader("History Tab (clock icon)");
        addText("Automatically saves all copy operations.");
        addEmpty();
        addSubheader("Entry Actions:");
        addBullet("X - Delete entry from history");
        addBullet("M (Material) - View materials for entry");
        addBullet("C (Clipboard) - Send to clipboard");
        addBullet("T (Tool) - Send directly to gadget");
        addEmpty();
        addBullet("Scroll with mouse wheel when many entries");
        addBullet("Clear All History button removes everything");
        addEmpty();
        
        addHeader("Trash Tab (trash can icon)");
        addText("Deleted schematics go here for recovery.");
        addEmpty();
        addSubheader("Features:");
        addBullet("Schematics are kept until you empty trash");
        addBullet("Shows original location for each item");
        addBullet("Scroll through trashed items with mouse wheel");
        addEmpty();
        addSubheader("Actions:");
        addBullet("R (Restore) - Recover to original folder location");
        addBullet("X (Delete) - Permanently remove from trash");
        addBullet("Empty Trash - Delete ALL trashed items permanently");
        addEmpty();
        
        addHeader("Settings Tab (gear icon)");
        addBullet("Max History Entries - Limit how many history entries are kept");
        addBullet("Max Trash Items - Limit how many deleted schematics are kept");
        addBullet("Trash Retention (days) - Auto-delete trash older than this");
        addBullet("Use +/- buttons to adjust values");
        addEmpty();
        ...
        addText("4. Click 'Save Schematic' to save with name and tags");
        addText("5. Later: Use Search to find schematics by name/tag");
        addText("6. Click T (Tool) icon to load schematic to gadget");
        addEmpty();
        
        addHeader("Quick Tips");
        addBullet("Hold the gadget when opening GUI for 'Copy from Tool'");
        addBullet("Use folders AND tags together for best organization");
        addBullet("Use Search tab to quickly find schematics across folders");
        addBullet("History is great for undo/versioning");
        addBullet("Material list helps plan resource gathering");
        addBullet("Deleted schematics go to Trash first - easy recovery!");
